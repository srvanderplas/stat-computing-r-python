...
Example from Telling Stories with Data: [Multilevel regression and post-stratification simulation with toddler bedtimes](https://www.tellingstorieswithdata.com/multilevel-regression-with-post-stratification.html#simulation---toddler-bedtimes).
...
Under scenario 1, our theoretical interval should be exp((-1.96/5, 1.96/5)) (because $\mu$ is 0, and $\sigma$ is 1, so $SE(\overline x) = 1/\sqrt{25} = 1/5$).
$(`r exp(-1.96/5)`,`r exp(1.96/5)`)$

Under scenario 2, the expected value of the lognormal distribution is $\exp(1/2) = `r exp(.5)`$, the variance is $(\exp(1) - 1)(\exp(1)) = `r (exp(1) - 1)*exp(1)`$ and our theoretical interval should be $(`r exp(.5) - 1.96*sqrt((exp(1) - 1)*exp(1))/5`, `r exp(.5) + 1.96*sqrt((exp(1) - 1)*exp(1))/5`)$.
This interval contains 0, which is implausible for lognormally distributed data.

Our expected values are different under scenario 1 and scenario 2: in scenario 1 we are computing an interval for $\mu$, in scenario 2, we are computing an interval for the population mean, which is $\exp(\mu + .5\sigma^2)$.
...
I wish we had time to cover purrr explicitly, but I at least want to expose you to how clean it makes your code.
:::
...
What if we just generate a bunch of uniform(0,1) variables, evaluate the value of the function at that point, and average the result?
...
[Example modified from this set of lecture notes](http://faculty.washington.edu/yenchic/17Sp_403/Lec2_MonteCarlo.pdf) [@chenLectureMonteCarlo2017]
:::
:::
