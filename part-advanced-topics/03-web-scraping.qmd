...
You may have noticed in @sec-demo-olympics-medals that the R code read the file in from a saved version instead of from the web directly. 
...
rev_war_battles = pd.read_html("https://en.wikipedia.org/wiki/List_of_American_Revolutionary_War_battles", attrs= {"class": "sortable wikitable"})[0]
...
battleCol = [i.select("td:first-child a") for i in rev_war_rows]
dateCol = [i.select(":nth-child(2)") for i in rev_war_rows]
locationCol = [i.select(":nth-child(3)") for i in rev_war_rows]

name = [i[0].string if len(i) > 0 else pd.NA for i in battleCol]
link = [i[0].attrs["href"] if len(i) > 0 else pd.NA for i in battleCol]
date = [i[0].string for i in dateCol]
location = [i[0].string for i in locationCol]

rev_war_battles = pd.DataFrame({"name": name, "link": link, "date": date, "location": location}).iloc[1:]
```
```{python}
#| eval: false
#| include: false
rev_war_battles.to_csv("../data/rev-war-battles2.csv")
```
```{python}
#| include: false
rev_war_battles=pd.read_csv("../data/rev-war-battles2.csv")
```
...
The second approach tends to be more successful (though the first is usually faster, if it works), and has the bonus effect of making you feel like a very powerful programmer (or mad scientist) as you sit back and watch your computer navigate the web according to your instructions, if you decide to launch the browser window to see the magic happen. 
Unfortunately, I can't easily demonstrate the visual magic of this approach without screen recordings that are bigger than I can put on GitHub. 
...
Now, we need to write a function to extract the info from the main page.
