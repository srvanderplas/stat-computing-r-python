...
-   `<table></table>` must contain the entire table specification, and indicates the start and end of table content.

-   `<tr> </tr>` defines a table row (hence, tr) and contains any column information for that row.

-   `<th>Heading Col 1</th>` defines a column heading with content 'Heading Col 1'.
    Sometimes, `<th>` elements contain attributes that indicate column width relative to the rest of the table.

-   `<td>Value 1</td>` defines a column that is not a heading.
    Its content is what is displayed in the column -- in this case, 'Value 1'.
