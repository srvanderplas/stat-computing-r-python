...
tables <- map2(files, page_numbers, ~extract_tables(.x, page = .y,  output = "tibble")[[1]])
...
cpi_data |>
  ...
  select(Category, report_date, Rel_importance) |>
  na.omit() |>
  ggplot(aes(x = report_date, y = Rel_importance, color = Category)) + geom_line() + 
