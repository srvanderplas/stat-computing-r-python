...
Many free and open-source OCR programs are built using the `tesseract` OCR engine.
Tesseract itself gives you much more control over the process, but requires converting each page of the PDF to an image (usually [PNG](../data/Lancaster-County-NE-Real-Estate-AppraisalCard-28749-2025-147568-pg1-image-1.png)) first, recognizing the text on each page, and then combining the text together to get [a text document](../data/Lancaster-County-NE-Real-Estate-AppraisalCard-28749-2025-147568-pg1-text-1.txt) (click the link to see the resulting text file).
This text document means you can search the PDF for text, but it doesn't provide all of the features that a hybrid PDF with both an image and a text layer provides -- the text isn't associated with the $(x,y)$ location on the page(s). 
 the image-only PDF to [PNG] and then using `tesseract` with the english-language flag to OCR

When I need OCR, I prefer to use a program called [`ocrmypdf`](https://github.com/ocrmypdf/OCRmyPDF) that is based on tesseract and available for most distributions.
The output of `ocrmypdf` is a [hybrid PDF](../data/Lancaster-County-NE-Real-Estate-AppraisalCard-28749-2025-147568-pg1-hybrid-ocrmypdf.pdf) that has the text and image data superimposed (the text is invisible until you highlight it). 
