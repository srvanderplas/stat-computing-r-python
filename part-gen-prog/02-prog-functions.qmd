...
Many of the instructions here modified from @makarovUsePythonRstudio2022.
...
This is by far the most common option for R users and it requires just about no setup on your part.
Just install packages with `install.packages()` if you're installing packages that are on CRAN, or `remotes::install_github()` if you're installing packages from github, and you're good to go, unless something goes wrong.

##### renv

renv is an attempt to bring virtual environments to R.
It's pretty useful for complex or long-term projects, but it can be pain for very simple and uncomplicated projects.
...
Once you've installed the package on your machine, you can comment these lines out so that they don't run every time - this makes it a bit easier if you try to run old code on a new machine, as you can just uncomment those lines.
:::
:::
