...
```{python vector-creation-python-1}
import numpy as np
...
```{python vector-creation-python-2, error = T}
...
```{r add-pandas, eval=TRUE, echo=FALSE, include = FALSE}
reticulate::install_python("pandas")
reticulate::install_python("lxml")
```

```{python series-pandas-1, error = F}
import pandas as pd
...
```{python read-state-pops, cache = F}
import pandas as pd
