...
Let's start with a totally absurd hypothetical situation: Suppose I watched the documentary ["Chicken People"](https://www.imdb.com/title/tt4819510/) and became interested in the different breeds of chicken. 
...
::: callout-demo
### Chicken Breed Data Assembly

::: panel-tabset

#### R

First, let's get the table of breed information.
I have re-hosted the HTML structure of the files to preserve the ability to use this example even when the website updates/changes. 

```{r}
library(rvest)
url <- "https://www.cacklehatchery.com/chicken-breeds/" # original
url <- "https://raw.githubusercontent.com/srvanderplas/datasets/main/raw/chickens/breeds-table.html"
...
The `html_table` method in rvest does not maintain the links to the breed pages, but we can get the list of links separately using some magic that I'm not going to explain here because it's web scraping.


```{r}
breeds <- breeds %>%
  mutate(name = str_replace_all(`Chicken Breed Name`, 
                                c("[[:punct:]]" = "", 
                                  " " = "-", 
                                  "[^A-z-]" = "")))
...
Then, we can apply a function that takes an individual breed URL and acquires additional data from the individual breed pages.
Again, I will not explain how this function is constructed here; instead we'll take its existence for granted.

```{r}
library(xml2)
parse_breed <- function(url) {
  html <- read_html(url)
  breed_facts <- html %>% html_nodes(".breed_facts_tab li") %>% html_nodes("strong")
  ...
  description <- html %>% html_nodes("#tab-description") %>% html_text() %>% str_trim() %>%
  ...
  availability <- html %>% html_node("#tab-availability table") %>% parse_availability()
  
  videos <- html %>% html_node(".video_tab") %>% html_nodes("iframe") %>% html_attr("src")
  ...
  reviews <- html %>% html_nodes(".comment") %>% map_df(parse_reviews)
...
additional_info <- map_df(breeds$link[1:10], parse_breed)
additional_info
